  ...
  //default to isTestnet false if nothing else is specified
    try {
        ...
        if (vChainMerkleBranch.size() > 30)
            throw runtime_error("Aux POW chain merkle branch too long");
        ...
        if (Block::checkMerkleBranch(getHash(), _merkleBranch, _index) != parentBlock.getMerkleRoot())
            throw runtime_error("Aux POW merkle root incorrect");
        ...
        if (pc == script.end())
            throw runtime_error("Aux POW missing chain merkle root in parent coinbase");
        
        if (pcHead != script.end()) {
            // Enforce only one chain merkle root by checking that a single instance of the merged
            // mining header exists just before.
            if (script.end() != std::search(pcHead + 1, script.end(), UBEGIN(pchMergedMiningHeader), UEND(pchMergedMiningHeader)))
                throw runtime_error("Multiple merged mining headers in coinbase");
            if (pcHead + sizeof(pchMergedMiningHeader) != pc)
                throw runtime_error("Merged mining header is not just before chain merkle root");
        }
        else {
            ...
            if (pc - script.begin() > 20)
                throw runtime_error("Aux POW chain merkle root must start in the first 20 bytes of the parent coinbase");
        }
        ...
        pc += vchRootHash.size();
        if (script.end() - pc < 8)
            throw runtime_error("Aux POW missing chain merkle tree size and nonce in parent coinbase");
        
        int nSize;
        memcpy(&nSize, &pc[0], 4);
        if (nSize != (1 << vChainMerkleBranch.size()))
            throw runtime_error("Aux POW merkle branch size does not match parent coinbase");
        ...
        if ((unsigned int)nChainIndex != (rand % nSize))
            throw runtime_error("Aux POW wrong index");
    }
    catch (std::exception& e) {
        log_error("Aux POW %s", e.what());
        return false;
    }
    return true;
}
